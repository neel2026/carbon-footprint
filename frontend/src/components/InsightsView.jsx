...
export const InsightsView = memo(({ insight, handleBack }) => {
  const dominantCategory = getHighestImpactCategory(insight.currentEntry.breakdown);
  const comparison = compareToAverage(insight.currentEntry.total, insight.profile.country);
    ...
    <section className="insights-view" aria-live="polite" aria-label="Carbon footprint insights">
      ...
      <div className="insights-content">
        ...
        <div className="insights-view__comparison">
          <p>You logged <strong>{insight.currentEntry.total.toFixed(1)} kg CO₂</strong> this week.</p>
          <p>Your largest category is <strong>{dominantCategory}</strong>. Your overall footprint is <strong>{comparison.percentile.toLowerCase()}</strong> relative to the {insight.profile.country} annual average.</p>
        </div>
        <ActionCard
